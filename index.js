...
const browseRandomArticles = async (page, browser) => {
  ...
  for (let i = 0; i < articlesToBrowse; i++) {
    ...
    const articleUrl = await article
      .$eval("a.jj-link.title", (node) => node.href)
      .catch(() => null);
    const title = await article
      .$eval("a.jj-link.title", (el) => el.textContent.trim())
      .catch(() => "标题获取失败");
...
const main = async () => {
  ...
  try {
    ...
    const login = async (retryCount = 0) => {
      ...
      // 等待二维码图片的容器出现
      await page
        .waitForSelector(".qrcode-img", { timeout: 5000 })
        .catch(async () => {
    ...
    while (attempt < maxRetries && !freeDrawFound && !alreadySignedIn) {
      ...
      try {
        ...
        if (signedinButton) {
          ...
          await page.waitForSelector(".code-calender .signin", {
            visible: true,
            timeout: 5000,
          });
      ...
      await delay(2000);
      await page.goto(
        "https://juejin.cn/user/center/lottery?from=sign_in_success",
        {
          waitUntil: "networkidle0",
        }
      );
      ...
      //新增是否已经免费抽奖判断
      try {
        await page.waitForSelector("#turntable-item-0 div.text-free", {
          visible: true,
          timeout: 5000,
        });
